...
# Create and close alert --------------------------------------------------
shinyApp(
  ui =
    fluidPage(
      sidebarLayout(
        sidebarPanel(
          textInput("num1", NULL, value = 100),
    ...
    function(input, output, session) {
      output$exampleOutput <- renderText({
        ...
        if (is.na(num1) | is.na(num2)) {
          createAlert(
            session, "alert", "exampleAlert", title = "Oops",
  ...
  server =
    function(input, output, session) {
      observeEvent(input$togOne, ({
        print(glue::glue("input$tagOne: {input$tagOne}"))
    ...
    fluidPage(
      sidebarLayout(
        sidebarPanel(
          ...
          selectInput(
            "styleSelect", "Select style for Panel 1",
        ...
        mainPanel(
          bsCollapse(
            ...
            bsCollapsePanel(
              "Panel 1", "This is a panel with just text ",
              "and has the default style. You can change the style in ",
              "the sidebar.", style = "info"
            ),
            bsCollapsePanel(
              "Panel 2", "This panel has a generic plot. ",
    ...
    fluidPage(
      sidebarLayout(
        sidebarPanel(
          ...
          bsTooltip(
            "bins", "The wait times will be broken into this many equally spaced bins",
    ...
    fluidPage(
      sidebarLayout(
        ...
        mainPanel(
          plotOutput("distPlot"),
          bsModal(
            "modalExample", "Data Table", "tabBut", size = "large",
